...
#define MODULE_VOTE_PREFIX "Match"
...
public void voteModulePluginStart()
{
    ...
    g_hModeMaxPlayers = CreateConVar("simpl_mode_maxplayers", "4", "模式加载完成后, 设置服务器位置为多少", CVAR_FLAG, true, 1.0, true, 31.0);
...
void startResetMatchVote(int client) {
    ...
    if (GetClientTeam(client) <= TEAM_SPECTATOR) {
        CPrintToChat(client, "[{G}%s{W}] 杂鱼旁观不能发起卸载配置投票捏❤", MODULE_VOTE_PREFIX);
        return;
    }
    if (!isModeLoaded) {
        CPrintToChat(client, "[{G}%s{W}] 当前未加载任何配置模式", MODULE_VOTE_PREFIX);
        return;
    }
    if (!IsNewBuiltinVoteAllowed()) {
        CPrintToChat(client, "[{G}%s{W}] 没看到正在进行的投票吗?", MODULE_VOTE_PREFIX);
        return;
    }
    ...
    if (connectCount > 0) {
        CPrintToChat(client, "[{G}%s{W}] 当前有 {O}%d {W}位正在连接中的玩家,不能发起卸载配置投票捏❤", MODULE_VOTE_PREFIX, connectCount);
        return;
    }
    ...
    // CPrintToChatAll("[{G}%s{W}] 玩家 {O}%N {G}发起了一个卸载当前模式的投票", MODULE_VOTE_PREFIX, client);
...
public int MatchModeMenuHandler(Menu menu, MenuAction action, int client, int item) {
    if (action == MenuAction_End) {
        ...
        if (modeKey.JumpToKey(info) && modeKey.GotoFirstSubKey()) {
            ...
            FormatEx(buffer, sizeof(buffer), "%s:", info);
            ...
            CPrintToChat(client, "[{G}%s{W}] 无法找到配置文件 {O}%s", MODULE_VOTE_PREFIX, info);
            showMatchModeMenu(client);
        }
    }
    return 0;
...
bool startMatchVote(int client, const char[] name) {
    ...
    if (!IsNewBuiltinVoteAllowed()) {
        CPrintToChat(client, "[{G}%s{W}] 当前不允许发起新的投票捏❤", MODULE_VOTE_PREFIX);
        return false;
    }
    ...
    if (index < g_hVotePlayerLimit.IntValue) {
        CPrintToChat(client, "[{G}%s{W}] 当前玩家数量小于允许发起投票的最小玩家数量 {O}%d", MODULE_VOTE_PREFIX, g_hVotePlayerLimit.IntValue);
    ...
    SetBuiltinVoteArgument(g_hVote, title);
    DisplayBuiltinVote(g_hVote, players, index, MENU_DISPLAY_TIME);
    // CPrintToChatAll("[{G}%s{W}] 玩家 {O}%N {G}发起了一个将模式更改为 {O}%s {G}的投票", MODULE_VOTE_PREFIX, client, name);
    CPrintToChatAll("[{G}%s{W}] {B}%N {W}杂鱼发起一个将模式更改为 {O}%s {W}的投票❤", MODULE_VOTE_PREFIX, client, name);
    return true;
}
...
public int VoteActionHandler(Handle vote, BuiltinVoteAction action, int item, int client) {
    switch (action) {
        case BuiltinVoteAction_End: {
            ...
            if (item == BUILTINVOTES_VOTE_YES) {
                char buffer[PLATFORM_MAX_PATH];
                FormatEx(buffer, sizeof(buffer), "正在加载 %s ❤❤❤", g_sCfg);
                DisplayBuiltinVotePass(vote, buffer);
