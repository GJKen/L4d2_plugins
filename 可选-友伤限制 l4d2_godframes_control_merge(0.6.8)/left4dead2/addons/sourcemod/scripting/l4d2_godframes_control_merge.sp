...
{
	g_hGodframeGlows = CreateConVar("gfc_godframe_glows", "1", "在 godframed 时更改幸存者的渲染(红色/透明)", _, true, 0.0, true, 1.0);
	g_hRageHittables = CreateConVar("gfc_hittable_rage_override", "1", "让坦克从可击中的命中中获得怒气.0=阻止怒气增益", _, true, 0.0, true, 1.0);
	g_hRageRock = CreateConVar("gfc_rock_rage_override", "1", "允许坦克从 godframed 命中中获得怒气,0=阻止怒气增益", _, true, 0.0, true, 1.0);
	g_hHittable = CreateConVar("gfc_hittable_override", "1", "允许 hittables 始终忽略 godframes", _, true, 0.0, true, 1.0);
	g_hRock = CreateConVar("gfc_rock_override", "0", "允许 hittables 始终忽略 godframes.", _, true, 0.0, true, 1.0);
	g_hWitch = CreateConVar("gfc_witch_override", "1", "让女巫总是无视 godframes.", _, true, 0.0, true, 1.0);
	g_hFF = CreateConVar("gfc_ff_min_time", "0.3", "允许 FF 合法的最短时间.", _, true, 0.0, true, 3.0);
	g_hSpit = CreateConVar("gfc_spit_extra_time", "0.7", "在吐出伤害之前允许额外的 Godframe 时间", 0, true, 0.0, true, 3.0);
	g_hCommon = CreateConVar("gfc_common_extra_time", "0.0", "允许普通伤害之前的额外 Godframes 时间", 0, true, 0.0, true, 3.0);
	g_hHunter = CreateConVar("gfc_hunter_duration", "2.1", "猛扑后的 Godframes 应该持续多久?", _, true, 0.0, true, 3.0);
	g_hJockey = CreateConVar("gfc_jockey_duration", "0.0", "骑行后 Godframes 应该持续多长时间?", _, true, 0.0, true, 3.0);
	g_hSmoker = CreateConVar("gfc_smoker_duration", "0.0", "拉动或窒息后 Godframes 应该持续多长时间?", _, true, 0.0, true, 3.0);
	g_hCharger = CreateConVar("gfc_charger_duration", "2.1", "击打后的 Godframes 应该持续多长时间?", _, true, 0.0, true, 3.0);
	g_hSpitFlags = CreateConVar("gfc_spit_zc_flags", "6", "哪些类别会受到额外吐痰保护时间的影响,1=hunter 2=smoker 4=jockey 8=charger", _, true, 0.0, true, 15.0);
	g_hCommonFlags= CreateConVar("gfc_common_zc_flags", "0", "哪些类别将受到额外公共保护时间的影响,1=hunter 2=smoker 4=jockey 8=charger", _, true, 0.0, true, 15.0);

	g_hCvarEnable = CreateConVar("l4d2_undoff_enable", "7", "位标志:启用插件功能(加在一起):1=太接近, 2=charger携带, 4=有罪机器人, 7=全部, 0=关闭", FCVAR_NOTIFY);
	g_hCvarBlockZeroDmg = CreateConVar("l4d2_undoff_blockzerodmg","7", "位标志:启用插件功能(加在一起):1=太接近, 2=charger携带, 4=有罪机器人, 7=全部, 0=关闭", FCVAR_NOTIFY);
g_hCvarBlockZeroDmg = CreateConVar("l4d2_undoff_blockzerodmg","7", "位标志:阻止0伤害友方火力效果, 如后坐力和发声/统计数据(加在一起):4=机器人击中人类阻止后坐力, 2=在所有难度上阻止人声/统计数据, 1=阻止除 Easy 之外的所有内容的人声/统计数据(标志 2 优先), 0=关闭", FCVAR_NOTIFY);
	g_hCvarPermDamageFraction = CreateConVar("l4d2_undoff_permdmgfrac", "1.0", "对永久健康造成的最小伤害", FCVAR_NOTIFY, true, 0.0, true, 1.0);

	g_hCvarEnableShotFF = CreateConVar("l4d2_shotgun_ff_enable", "1", "启用 Shotgun FF 模块?", _, true, 0.0, true, 5.0);
	g_hCvarModifier = CreateConVar("l4d2_shotgun_ff_multi", "0.5", "霰弹枪 FF 伤害修正值", _, true, 0.0, true, 5.0);
	g_hCvarMinFF = CreateConVar("l4d2_shotgun_ff_min", "1.0", "允许的最小霰弹枪 FF 伤害,0 表示无限制", _, true, 0.0);
	g_hCvarMaxFF = CreateConVar("l4d2_shotgun_ff_max", "6.0", "允许最大霰弹枪 FF 伤害,0 表示无限制", _, true, 0.0);
