...
any Native_GetHeadShotCount(Handle plugin, int numParams)
{
	int client = GetNativeCell(1);
	if (client < 1 || client > MaxClients)
	{
	...
	return GetHeadShot(client);
}

int Native_GetFFCount(Handle plugin, int numParams)
{
	int client = GetNativeCell(1);
	if (client < 1 || client > MaxClients)
	{
	...
	return player[client].ClientFFCount;
}

int Native_GetGotFFCount(Handle plugin, int numParams)
{
	int client = GetNativeCell(1);
	if (client < 1 || client > MaxClients)
	{
...
{ 
	...
	cv_Medical1 = CreateConVar("l4d2_yaoyi_enable", "1", "是否允许药役限制物品购买 不允许:0 允许:1", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	cv_Weapon = CreateConVar("l4d2_weapon_enable", "1", "大枪购买开关 开:1 关:0", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	cv_AmmoTime = CreateConVar("l4d2_give_ammo_time", "133.0", "补充子弹的最小间隔时间,小于0.0关闭功能");
...
void SQL_SaveHeadShotCount(int client)
{
	if (!sqlite)
		return;

	char query[1024];
	FormatEx(query, sizeof query, "UPDATE Shop SET HeadShotCount = %d WHERE SteamID = '%s';", player[client].ClientHeadShotCount, GetSteamId(client));
	SQL_FastQuery(sqlite, query);
}

void SQL_SaveFFCount(int client)
{
	if (!sqlite)
		return;

	char query[1024];
	FormatEx(query, sizeof query, "UPDATE Shop SET FFCount = %d WHERE SteamID = '%s';", player[client].ClientFFCount, GetSteamId(client));
	SQL_FastQuery(sqlite, query);
}

void SQL_SaveGotFFCount(int client)
{
	if (!sqlite)
		return;

	char query[1024];
	FormatEx(query, sizeof query, "UPDATE Shop SET GotFFCount = %d WHERE SteamID = '%s';", player[client].ClientGotFFCount, GetSteamId(client));
	SQL_FastQuery(sqlite, query);
}
...
float GetHeadShot(int client)
{
	float accuracy = (player[client].ClientKillCI + player[client].ClientKillSI) == 0 ? 0.0 : float(player[client].ClientHeadShotCount)/float((player[client].ClientKillCI + player[client].ClientKillSI));
...
//枪械物品菜单
public void WeaponsMenu(int client) 
{
	Menu menu = new Menu(WeaponsMenu_back);
	menu.SetTitle("点数(剩余:%d)\n————————", player[client].ClientPoint);
	menu.AddItem("rifle", "M16(1点)");
	menu.AddItem("rifle_ak47", "AK(1点)");
	menu.AddItem("rifle_desert", "SCAR三连发(1点)");
	menu.AddItem("autoshotgun", "一代连喷(1点)");
	menu.AddItem("shotgun_spas", "二代连喷(1点)");
	menu.AddItem("hunting_rifle", "猎狙(1点)");
	menu.AddItem("sniper_military", "军狙(1点)");
	menu.AddItem("weapon_sniper_awp", "AWP(1点)");
	menu.AddItem("weapon_sniper_scout", "Scout(1点)");
	menu.AddItem("weapon_rifle_sg552", "SG552(1点)");
	menu.Display(client, 20);
} 

//杂项物品菜单选择后执行
public int WeaponsMenu_back(Menu menu, MenuAction action, int client, int param)
{
	if(judge(client))
		return 0;
	if(player[client].ClientPoint == 0){
	...
	if (action == MenuAction_Select)
	{
		...
		{ 
			case 0: //M16
			{
				GiveCommand(client, "rifle");
				PrintWeaponsName(client, 0);
			}
			case 1: //AK
			{ 
				GiveCommand(client, "rifle_ak47");
				PrintWeaponsName(client, 1);
			}
			case 2: //三连发
			{ 
				GiveCommand(client, "rifle_desert");
				PrintWeaponsName(client, 2);
			}
			case 3: //一代连
			{ 
				GiveCommand(client, "autoshotgun");
				PrintWeaponsName(client, 3);
			}
			case 4: //二代连
			{ 
				GiveCommand(client, "shotgun_spas");
				PrintWeaponsName(client, 4);
			}
			case 5: //猎狙
			{ 
				GiveCommand(client, "hunting_rifle");
				PrintWeaponsName(client, 5);
			}
			case 6: //军狙
			{ 
				GiveCommand(client, "sniper_military");
				PrintWeaponsName(client, 6);
			}
			case 7: //AWP
			{ 
				GiveCommand(client, "weapon_sniper_awp");
				PrintWeaponsName(client, 7);
			}
			case 8: //Scout
			{ 
				GiveCommand(client, "weapon_sniper_scout");
				PrintWeaponsName(client, 8);
			}
			case 9: //SG552
			{ 
				GiveCommand(client, "weapon_rifle_sg552");
				PrintWeaponsName(client, 9);
			}
...
//武器物品聊天框展示
void PrintWeaponsName(int client, int i)
{
	char WeaponsName[][] = {"M16", "AK", "三连发", "一代连喷", "二代连喷", "猎狙", "军狙", "AWP", "Scout", "SG552"};
	player[client].ClientPoint--;
	SQL_SavePoint(client);
	PrintToChat(client, "\x04[提示]\x05购买\x03%s\x05成功,还剩\x04%d\x05点数.", WeaponsName[i], player[client].ClientPoint);
}
...
//判断玩家
bool NoValidPlayer(int Client, bool bot=true)
{
	if (Client < 1 || Client > MaxClients)
		return true;
