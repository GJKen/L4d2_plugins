...
{
	g_hCvarFFBlock = CreateConVar("l4d2_block_infected_ff", "1", "禁用 SI->SI 友军火力", _, true, 0.0, true, 1.0);
	g_hCvarAllowTankFF = CreateConVar("l4d2_infected_ff_allow_tank", "1", "不要禁用其他 SI 上的坦克友军火力", _, true, 0.0, true, 1.0);
	g_hCvarBlockWitchFF = CreateConVar("l4d2_infected_ff_block_witch", "0", "禁用针对开关的FF", _, true, 0.0, true, 1.0);
