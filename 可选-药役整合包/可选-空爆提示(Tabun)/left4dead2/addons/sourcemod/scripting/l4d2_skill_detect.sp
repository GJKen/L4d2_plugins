...
stock HandleLevel( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★ {blue}%N {default}使用近战{blue}满血秒杀{default}正在冲锋的 {blue}%N", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★★ {blue}%N {default}使用近战{blue}满血秒杀{default}正在冲锋的 {blue}牛", attacker );
        }
    }
...
stock HandleLevelHurt( attacker, victim, damage )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★ {blue}%N {default}使用近战{blue}砍死{default}正在冲锋的 {blue}%N {default}({blue}%i 伤害{default})", attacker, victim, damage );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★ {blue}%N {default}使用近战{blue}砍死{default}正在冲锋的 {blue}牛 ({blue}%i 伤害{default})", attacker, damage );
        }
    }
...
stock HandleDeadstop( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        else if ( IS_VALID_INGAME(attacker) && g_deadstop == false)
        {
...
HandleTongueCut( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{green}★★★ {blue}%N {default}砍断 {blue}%N{default}的舌头", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{green}★★ {blue}%N {default}砍断 {blue}smoker{default}的舌头", attacker );
        }
    }
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)) 
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
        	CPrintToChatAll( "{green}★★ {blue}%N {default}成功从 {blue}%N{default}的舌头下{blue} %s自救", attacker, victim, (withShove) ? " 用推" : "" );
        ...
        {
        	CPrintToChatAll( "{green}★★ {blue}%N {default}成功从 {blue}smoker{default}的舌头下{blue} %s自救", attacker, (withShove) ? " 用推" : "" );
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        */
        CPrintToChatAll( "{green}★ {blue}%N {default}爆了一个{blue}tank石头", attacker );
    }
...
stock HandleClear( attacker, victim, pinVictim, zombieClass, Float:clearTimeA, Float:clearTimeB, bool:bWithShove = false )
{
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport))
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime && !GetConVarBool(g_hCvarReport))
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
