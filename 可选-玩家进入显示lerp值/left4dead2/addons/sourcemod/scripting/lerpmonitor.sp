...
{
	cVarAllowedLerpChanges = CreateConVar("sm_allowed_lerp_changes", "4", "允许的一半lerp 更改数", _, true, 0.0, true, 20.0);
	cVarLerpChangeSpec = CreateConVar("sm_lerp_change_spec", "1", "在超过lerp change count时移动到旁观者?", _, true, 0.0, true, 1.0);
	cVarReadyUpLerpChanges = CreateConVar("sm_readyup_lerp_changes", "1", "允许在准备就绪时更改lerp,ready-up", _, true, 0.0, true, 1.0);
	cVarShowLerpTeamChange = CreateConVar("sm_show_lerp_team_changes", "1", "如果玩家改变球队，则显示有关玩家 lerp 的消息", _, true, 0.0, true, 1.0);
	cVarMinLerp = CreateConVar("sm_min_lerp", "0.000", "最小允许的 lerp value", _, true, 0.000, true, 0.500);
	cVarMaxLerp = CreateConVar("sm_max_lerp", "100.0", "最大允许的 lerp value[对抗0.067]", _, true, 0.000, true, 0.500);
...
void ProcessPlayerLerp(int client, bool load = false, bool team = false) 
{
	...
	if ((FloatCompare(newLerpTime, cVarMinLerp.FloatValue) == -1)  || (FloatCompare(newLerpTime, cVarMaxLerp.FloatValue) == 1)) {
		//PrintToChatAll("%N's lerp 更改为 %.01f", client, newLerpTime * 1000);
		if (!load) {
			...
			CPrintToChatAllEx(client, "{default}<{olive}Lerp{default}> {teamcolor}%N {default}移至旁观者 lerp {teamcolor}%.01f", client, newLerpTime * 1000);
			ChangeClientTeam(client, L4D_TEAM_SPECTATE);
			CPrintToChatEx(client, client, "{default}<{olive}Lerp{default}> 非法的lerp值 (min: {teamcolor}%.01f{default}, max: {teamcolor}%.01f{default})", cVarMinLerp.FloatValue * 1000, cVarMaxLerp.FloatValue * 1000);
		}
	...
	if (isMatchLife || !cVarReadyUpLerpChanges.BoolValue) { // Midgame?
		...
		if (cVarLerpChangeSpec.BoolValue && (count > max)) {
			CPrintToChatAllEx(client, "{default}<{olive}Lerp{default}> {teamcolor}%N {default}移至旁观者(非法lerp change)!", client);
			ChangeClientTeam(client, L4D_TEAM_SPECTATE);
			CPrintToChatEx(client, client, "{default}<{olive}Lerp{default}> Lerp midgame的非法更改! 将其改回 {teamcolor}%.01f", currentLerpTime * 1000);
			// no lerp update
			return;
		}
