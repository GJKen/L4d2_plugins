...
public void OnPluginStart() {
	...
	g_cBotLimit =				CreateConVar("bots_limit",				"5",		"开局Bot的数量", CVAR_FLAGS, true, 1.0, true, float(MaxClients));
	...
	RegConsoleCmd("sm_afk",				cmdGoIdle,		"打酱油");
	RegConsoleCmd("sm_teams",			cmdTeamPanel,	"团队菜单");
	RegConsoleCmd("sm_join",			cmdJoinTeam2,	"加入射精者");
	RegConsoleCmd("sm_jg",			cmdJoinTeam2,	"加入射精者");
	RegConsoleCmd("sm_tkbot",			cmdTakeOverBot,	"夺舍指定智障");

	RegConsoleCmd("sm_away",				cmdJoinTeam1,	"加入撸管人");
	RegConsoleCmd("sm_a",				cmdJoinTeam1,	"加入撸管人");
	RegAdminCmd("sm_bot",				cmdBotSet,		ADMFLAG_ROOT,	"设置开局智障数量");
...
bool JoinSurTeam(int client) {
	...
	if (!bot && !canRespawn) {
		...
		PrintToChat(client, "\x05重复加入默认为 \x04嗝屁状态\x01");
		return true;
	}
...
void TakeOverBotMenu(int client) {
	...
	for (int i = 1; i <= MaxClients; i++) {
		...
		FormatEx(disp, sizeof disp, "%s - %s", IsPlayerAlive(i) ? "存活" : "嗝屁", g_sSurvivorNames[GetCharacter(i)]);
		...
		case MenuAction_Select: {
			...
			if (item[0] == 'o') {
				...
				if (bot > 0 && bot <= MaxClients && IsValidSurBot(bot)) {
					...
					PrintToChat(param1, "当前撸管人目标非可夺舍智障");
			}
			else {
				bot = GetClientOfUserId(StringToInt(item));
				if (!bot || !IsValidSurBot(bot))
					PrintToChat(param1, "选定的目标智障已失效");
				else {
...
void DrawTeamPanel(int client, bool autoRefresh) {
	...
	panel.SetTitle("团队信息|Team Info");

	static char info[MAX_NAME_LENGTH];
	static char name[MAX_NAME_LENGTH];

	FormatEx(info, sizeof info, "上帝 [%d]", GetTeamPlayers(TEAM_SPECTATOR, false));
	panel.DrawItem(info);
	...
	for (; i <= MaxClients; i++) {
		...
		FormatEx(info, sizeof info, "%s - %s", GetBotOfIdlePlayer(i) ? "打酱油" : "撸管人", name);
		panel.DrawText(info);
	}
//g_iBotLimit 可选参数,意为bot总数
	...
	for (i = 1; i <= MaxClients; i++) {
		...
		if (!IsPlayerAlive(i))
			FormatEx(info, sizeof info, "嗝屁 - %s", name);
		else {
			if (GetEntProp(i, Prop_Send, "m_isIncapacitated"))
				FormatEx(info, sizeof info, "躺平 - %dHP - %s", GetClientHealth(i) + GetTempHealth(i), name);
			else if (GetEntProp(i, Prop_Send, "m_currentReviveCount") >= maxInc)
				FormatEx(info, sizeof info, "阴阳 - %dHP - %s", GetClientHealth(i) + GetTempHealth(i), name);
			else
	...
	if (num) {
		...
		for (i = 0; i < num; i++) {
			...
			if (IsPlayerAlive(zombie.client)) {
				...
				FormatEx(info, sizeof info, "(%s)嗝屁 - %s", ZombieName[zombie.class - 1], name);
