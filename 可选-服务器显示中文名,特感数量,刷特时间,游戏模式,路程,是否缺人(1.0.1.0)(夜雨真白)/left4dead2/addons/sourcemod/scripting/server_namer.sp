...
// ConVars
//ConVar cv_hInfectedTime, cv_hInfectedLimit;
...
public void OnPluginStart()
{
	...
	// CreateConVars
	g_hAllowDisplayInfectedInfo = CreateConVar("sn_display_infected_info", "0", "是否在服名中显示特感信息", CVAR_FLAG, true, 0.0, true, 1.0);
	...
	g_hAllowDisplayNeedPeople = CreateConVar("sn_display_need_people", "0", "是否在当前服名中显示是否缺人", CVAR_FLAG, true, 0.0, true, 1.0);
	g_hRefreshTime = CreateConVar("sn_refresh_time", "30", "服名的刷新时间，单位：秒", CVAR_FLAG, true, 0.1);
	g_hBaseServerName = CreateConVar("sn_base_server_name", "", "基本服名，配置则使用当前服名，未配置则使用文件中的服名", CVAR_FLAG, true, 0.0);
	g_hBaseModeName = CreateConVar("sn_base_mode_name", "a", "基本模式名称", CVAR_FLAG, true, 0.0);
...
void setServerName()
{
	...
	{
	Format(modeName,sizeof(modeName),"纯净模式");
	}
	...
	{
	Format(modeName,sizeof(modeName),"单人模式");
	}
	...
	{
	Format(modeName,sizeof(modeName),"普通模式");
	}
